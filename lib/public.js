...
class PublicClient {
   ...
   * @example
   * publicClient.cb(
   *   { _method: 'getLoanOrders', currency: 'BTC' },
   *   (error, data) => {
   *     if (error) {
   *       console.error(error);
   *     } else {
   ...
   * );
   */
  cb({ _method, ...options } = {}, callback) {
    try {
      this[_method].call(this, options).then(data => callback(null, data));
    } catch (error) {
      callback(error);
    }
  }
    ...
    for (let key of keys) {
      if (options[key] === undefined) {
        if (remove) {
          ...
          let error = '`options` is missing a required property: `' + key + '`';
