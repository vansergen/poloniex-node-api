...
class AuthenticatedClient extends PublicClient {
  ...
  constructor({ key, secret, ...other }) {
    PublicClient.filterProperties({ key, secret });
    super(other);
  ...
  post({ ...form } = {}) {
    const { command } = form;
    AuthenticatedClient.filterProperties({ command });
    AuthenticatedClient.filterProperties(form, true);

    form.nonce = this._nonce();
    const auth = { key: this.key, secret: this.secret };
    const options = { form, url: this.api_uri + '/tradingApi', method: 'POST' };
    options.headers = SignRequest(auth, options);
    return this.request(options);
  }

  /**
   * @example
   * const balances = authClient.getBalances();
   ...
   * @example
   * const balances = authClient.getCompleteBalances({ account: 'all' });
  ...
  /**
   * @example
   * const addresses = authClient.getDepositAddresses();
   ...
   * @example
   * const addresses = authClient.getNewAddress({ currency: 'BTC' });
  ...
  getNewAddress({ currency } = {}) {
    AuthenticatedClient.filterProperties({ currency });
   ...
   * @example
   * const deposits = authClient.getDepositsWithdrawals({
   *   start: 1539952118,
   *   end: 1540318271,
   * });
  ...
  getDepositsWithdrawals({ start, end } = {}) {
    AuthenticatedClient.filterProperties({ start, end });
   ...
   * @example
   * const orders = authClient.getOpenOrders();
   ...
   * @example
   * const trades = authClient.getHistoryTrades();
